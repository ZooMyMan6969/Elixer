...
-- /scripts/#api/loot_manager.lua

function loot_manager.is_lootable_item(obj, exclude_potions, exclude_gold)
    -- Existing logic to identify lootable items
    local is_lootable = -- existing conditions

    -- Check if the item is a potion and if potions should not be excluded
    if not exclude_potions and loot_manager.is_potion(obj) then
        is_lootable = true
    end

    -- Check if the item is gold and if gold should not be excluded
    if not exclude_gold and loot_manager.is_gold(obj) then
        is_lootable = true
    end
